...
set of panes that tile the view (see `ui/screen.py`).

Each pane owns its drawing code. The `Pane` base carries the same `config`
shortcuts the old `LayoutManager` exposed (`self.display`, `self.weather`,
`self.subway`, `self.time`) so the drawing methods read naturally; cross-pane
chrome (the dividers) still lives in `LayoutManager._draw_sections`.
"""
...
class Pane:
    ...
    def __init__(self, rect: tuple[int, int, int, int]):
        ...
        # Config shortcuts, mirroring the old LayoutManager, so the relocated
        # drawing code can keep using self.display / weather / subway / time.
